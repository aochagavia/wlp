...
To calculate the paths in a while, we make a path where the condition is
initially false and a path where the condition is initially false, the body is
executed and a new path through the while loop is executed. By doing this
rescursively, we can obtain the paths in the body and unroll the loop as much
as possible:
...
case, the body of the while is not executed so the path is empty. As in the
...
We need QuickCheck to generate only relevant test cases. We are looking for a
counterexample that makes the program fail. When the precondition does not hold
...
case irrelevant when it has:

\begin{itemize}
\item Invalid precondition: useless, since we are looking for a counterexample.
\item Trivial postcondition: useless, since we are looking for a counterexample.
\end{itemize}
...
out empty for some variable, QuickCheck will raise an error so we will need to
check the ranges before continuing with the test.
